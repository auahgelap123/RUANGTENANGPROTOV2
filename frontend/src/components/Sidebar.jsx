...
const Sidebar = () => {
  const { 
    getUsers, users, selectedUser, setSelectedUser, isUsersLoading, unreadMessages, 
    subscribeToMessages, unsubscribeFromMessages, sendRequest, acceptRequest, removeContact 
  } = useChatStore();
  ...
  useEffect(() => {
    ...
    fetchLeaderboard(); 
    subscribeToMessages();
    if (socket) subscribeToGameEvents();
  ...
  const availableGames = [
      { id: "TICTACTOE", name: "Tic Tac Toe", desc: "Adu strategi", icon: "❌" },
  ...
  return (
    <> 
      <aside className={`h-full border-base-300 flex flex-col transition-all duration-200 
        ${selectedUser ? "w-0 border-none lg:w-72 lg:border-solid lg:border-r" : "w-full border-r lg:w-72"}`}>
        
        {/* --- BOTTOM NAV (MOBILE) & TOP TABS (DESKTOP) --- */}
        <div className="fixed bottom-0 left-0 w-full bg-base-100 border-t border-base-300 flex justify-around z-[50] lg:static lg:border-t-0 lg:border-b pb-2 pt-1 lg:p-0">
          <button 
            onClick={() => { setActiveTab("pendengar"); setShowExplore(false); setSelectedUser(null); }} 
            className={`flex-1 pt-2 pb-1 lg:p-3 flex flex-col lg:flex-row items-center justify-center gap-1 ${!showExplore && activeTab === "pendengar" ? "text-primary border-t-2 border-primary lg:border-t-0 lg:border-b-2" : "text-zinc-500 border-t-2 border-transparent lg:border-none"}`}
          >
              <MessageCircle className="size-5 lg:size-6" />
              <span className="text-[10px] font-bold lg:hidden">Chat</span>
          </button>
          
          <button 
            onClick={() => { setActiveTab("pendengar"); setShowExplore(true); setSelectedUser(null); }} 
            className={`flex-1 pt-2 pb-1 lg:p-3 flex flex-col lg:flex-row items-center justify-center gap-1 ${showExplore && activeTab === "pendengar" ? "text-primary border-t-2 border-primary lg:border-t-0 lg:border-b-2" : "text-zinc-500 border-t-2 border-transparent lg:border-none"}`}
          >
              <Users className="size-5 lg:size-6" />
              <span className="text-[10px] font-bold lg:hidden">Explore</span>
          </button>
          
          <button 
            onClick={() => { setActiveTab("aktivitas"); setSelectedUser(null); }} 
            className={`flex-1 pt-2 pb-1 lg:p-3 flex flex-col lg:flex-row items-center justify-center gap-1 ${activeTab === "aktivitas" ? "text-primary border-t-2 border-primary lg:border-t-0 lg:border-b-2" : "text-zinc-500 border-t-2 border-transparent lg:border-none"}`}
          >
              <Map className="size-5 lg:size-6" />
              <span className="text-[10px] font-bold lg:hidden">Quest</span>
          </button>
          
          <button 
            onClick={() => { setActiveTab("arcade"); setSelectedUser(null); }} 
            className={`flex-1 pt-2 pb-1 lg:p-3 flex flex-col lg:flex-row items-center justify-center gap-1 ${activeTab === "arcade" ? "text-primary border-t-2 border-primary lg:border-t-0 lg:border-b-2" : "text-zinc-500 border-t-2 border-transparent lg:border-none"}`}
          >
              <Gamepad2 className="size-5 lg:size-6" />
              <span className="text-[10px] font-bold lg:hidden">Arcade</span>
          </button>
        </div>

        {/* --- WRAPPER KONTEN SIDEBAR --- */}
        <div className={`flex-1 flex-col overflow-y-auto w-full pb-[70px] lg:pb-0 ${selectedUser ? "hidden lg:flex" : "flex"}`}>
          ...
          {activeTab === "pendengar" && (
            <>
                            ...
                            <div className="mb-4 bg-base-200 rounded-lg p-2">
                                ...
                                {incomingRequests.map(user => (
                                    <div key={user._id} className="flex items-center gap-2 p-2 bg-base-100 rounded-md mb-1 shadow-sm">
                                        <img src={user.profilePic || "/avatar.png"} className="size-8 rounded-full object-cover shrink-0" />
                                        <div className="flex-1 min-w-0 font-medium text-sm truncate">{user.fullName}</div>
                                        <button onClick={() => acceptRequest(user._id)} className="btn btn-xs btn-primary btn-square shrink-0"><Check className="size-4"/></button>
                                        <button onClick={() => removeContact(user._id)} className="btn btn-xs btn-ghost btn-square text-error shrink-0"><X className="size-4"/></button>
                                    </div>
                                ))}
                            </div>
                        )}
                        ...
                        {filteredContacts.map((user) => (
                            <div key={user._id} onClick={() => setSelectedUser(user)} className={`w-full p-3 flex items-center gap-3 hover:bg-base-300 cursor-pointer rounded-lg group relative ${selectedUser?._id === user._id ? "bg-base-300" : ""}`}>
                                <div className="relative shrink-0">
                                    <img src={user.profilePic || "/avatar.png"} className={`size-12 rounded-full object-cover border-2 ${user.role==='psychologist'?'border-yellow-500':user.role==='volunteer'?'border-primary':'border-transparent'}`} />
                                ...
                                <div className="flex flex-col min-w-0 flex-1">
                                    <div className="flex justify-between items-center w-full">
                                        <div className="font-medium text-sm truncate flex gap-1 items-center max-w-[80%]">
                                            <span className="truncate">{user.fullName}</span>
                                            {user.role === 'psychologist' && <GraduationCap className="size-3 text-yellow-500 shrink-0"/>}
                                            {user.role === 'volunteer' && <ShieldCheck className="size-3 text-primary shrink-0"/>}
                                        </div>
                                        {unreadMessages[user._id] > 0 && <span className="badge badge-sm badge-error text-white font-bold shrink-0">{unreadMessages[user._id]}</span>}
                                    </div>
                                    
                                    <div className="text-xs text-zinc-400 flex items-center gap-2 mt-0.5 truncate">
                                        <span className="truncate">{user.role === 'psychologist' ? 'Psikolog' : user.role === 'volunteer' ? 'Relawan' : user.role === 'admin' ? 'Admin' : onlineUsers.includes(user._id) ? "Online" : "Offline"}</span>
                                        {user.rating > 0 && (<span className="flex items-center text-yellow-500 font-bold bg-yellow-500/10 px-1.5 py-0.5 rounded shrink-0"><Star className="size-3 fill-yellow-500 mr-1" />{user.rating}</span>)}
                                    </div>
                                </div>
                    ...
                    <>
                        {exploreHelpers.map((user) => (
                            <div key={user._id} className="card bg-base-200 p-3 mb-2 flex flex-row items-center gap-3">
                                <img src={user.profilePic || "/avatar.png"} className={`size-10 rounded-full border-2 shrink-0 ${user.role==='psychologist'?'border-yellow-500':'border-primary'}`} />
                                <div className="flex-1 min-w-0">
                                    <div className="font-bold text-sm truncate flex items-center gap-1">
                                        <span className="truncate">{user.fullName}</span>
                                        {user.role==='psychologist'?<GraduationCap className="size-3 text-yellow-500 shrink-0"/>:<ShieldCheck className="size-3 text-primary shrink-0"/>}
                                    </div>
                                    <div className="text-xs text-zinc-500">{user.role==='psychologist'?'Psikolog':'Volunteer'}</div>
                                </div>
                                {user.friendRequests?.includes(authUser._id) ? <span className="text-xs text-zinc-400 flex gap-1 items-center shrink-0"><Clock className="size-3"/> Sent</span> : <button onClick={() => sendRequest(user._id)} className="btn btn-sm btn-circle btn-primary btn-outline shrink-0"><UserPlus className="size-4"/></button>}
                            </div>
                        ))}
                        ...
                        {exploreOthers.map((user) => (
                            <div key={user._id} className="flex items-center gap-3 p-2 hover:bg-base-200 rounded-lg">
                                <img src={user.profilePic || "/avatar.png"} className="size-10 rounded-full shrink-0" />
                                <div className="flex-1 min-w-0">
                                    <div className="font-medium text-sm truncate">{user.fullName}</div>
                                    <div className="text-xs text-zinc-400">User</div>
                                </div>
                                {user.friendRequests?.includes(authUser._id) ? <span className="text-xs text-zinc-400 flex gap-1 items-center shrink-0"><Clock className="size-3"/> Sent</span> : <button onClick={() => sendRequest(user._id)} className="btn btn-sm btn-ghost hover:text-primary shrink-0"><UserPlus className="size-4"/></button>}
                            </div>
                        ))}
                    </>
                )}
              ...
              <div className="p-4 border-t border-base-300">
                <button onClick={() => setIsVolunteerModalOpen(true)} className="btn btn-primary btn-outline btn-sm w-full gap-2">
                    <HeartHandshake className="size-4 shrink-0" />
                    <span className="truncate">Jadilah Pendengar</span>
                </button>
              </div>
            </>
          )}
          ...
          {activeTab === "aktivitas" && (
            <div className="p-4 overflow-y-auto h-full w-full">
               ...
               <div className="space-y-3">
                  {quests.map((quest) => (
                    <div key={quest.id} className="card bg-base-200 shadow-sm border border-base-300 p-3 relative group w-full">
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex flex-col min-w-0 pr-2">
                                    <h4 className="font-bold text-sm truncate">{quest.title}</h4>
                                    <div className="flex gap-1 mt-1 shrink-0">
                                    <span className="badge badge-accent badge-xs">{quest.exp} XP</span>
                                  <span className="badge badge-warning text-white badge-xs">{quest.points || 0} ⚡</span>
                             </div>
                        </div>
                        <button onClick={(e) => { e.stopPropagation(); handleReroll(quest.id); }} className="btn btn-xs btn-circle btn-ghost text-zinc-400 hover:text-primary shrink-0"><RefreshCcw className="size-4" /></button>
                      </div>
                      <p className="text-xs text-zinc-500 mb-4 line-clamp-2">{quest.description}</p>
                      <button onClick={() => { setSelectedQuest(quest); setQuestInput(""); setQuestImage(null); }} className="btn btn-sm btn-outline btn-primary w-full gap-2">
                          {quest.questType === 'image' ? <Camera className="size-4"/> : <Send className="size-4"/>} 
                          <span className="truncate">Kerjakan</span>
                      </button>
                    </div>
                  ))}
          ...
          {/* TAB ARCADE */}
          {activeTab === "arcade" && (
              <div className="p-4 overflow-y-auto h-full flex flex-col gap-6 w-full">
                  <div className="bg-gradient-to-br from-primary/10 to-secondary/10 p-4 rounded-xl border border-primary/20">
                      <h3 className="font-bold text-lg text-primary flex items-center gap-2 mb-3"><Trophy className="size-5 text-yellow-500 shrink-0"/> Hall of Fame</h3>
                      <div className="space-y-2">
                          {leaderboard.map((user, idx) => (
                              <div key={user._id} className="flex items-center gap-3 bg-base-100 p-2 rounded-lg shadow-sm border border-base-200">
                                  <div className={`font-black w-6 text-center shrink-0 ${idx===0?'text-yellow-500 text-xl':idx===1?'text-zinc-400 text-lg':idx===2?'text-orange-400 text-lg':'text-zinc-500'}`}>{idx+1}</div>
                                  <img src={user.profilePic || "/avatar.png"} className="size-8 rounded-full object-cover border shrink-0"/>
                                  <div className="flex-1 min-w-0 font-medium text-sm truncate">{user.fullName}</div>
                                  <div className="badge badge-ghost font-bold gap-1 shrink-0">{user.gameWins || 0} <Crown className="size-3 text-yellow-500"/></div>
                              </div>
                          ))}
                  ...
                  <div>
                      <h3 className="font-bold text-lg text-base-content mb-3 flex items-center gap-2"><Gamepad2 className="size-5 shrink-0"/> Game Library</h3>
                      <div className="grid gap-3">
                          {availableGames.map((game) => (
                              <button key={game.id} onClick={() => setSelectedGameToPlay(game)} className="card bg-base-200 hover:bg-base-300 transition-colors p-3 border border-base-300 text-left flex flex-row items-center gap-4 group">
                                  <div className="text-3xl grayscale group-hover:grayscale-0 transition-all shrink-0">{game.icon}</div>
                                  <div className="flex-1 min-w-0">
                                      <div className="font-bold text-sm truncate">{game.name}</div>
                                      <div className="text-xs text-zinc-500 truncate">{game.desc}</div>
                                  </div>
                                  <div className="btn btn-sm btn-circle btn-primary btn-outline shrink-0"><Swords className="size-4"/></div>
                              </button>
                          ))}
                      </div>
                  </div>
      ...
      {selectedGameToPlay && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
              <div className="modal-box bg-base-100 max-w-sm">
                  ...
                  <h3 className="font-bold text-lg mb-1 truncate">Main {selectedGameToPlay.name}</h3>
                  ...
                  <div className="max-h-60 overflow-y-auto space-y-2 mb-4">
                      {myContacts.length === 0 ? <div className="text-center py-4 text-zinc-500 text-sm">Belum ada teman online.</div> : myContacts.map(friend => (
                          <button key={friend._id} onClick={() => { sendInvite(friend._id, selectedGameToPlay.id); setSelectedGameToPlay(null); }} className="w-full flex items-center gap-3 p-2 hover:bg-base-200 rounded-lg border border-transparent hover:border-base-300 transition-all">
                              <div className="relative shrink-0"><img src={friend.profilePic || "/avatar.png"} className="size-10 rounded-full object-cover"/>{onlineUsers.includes(friend._id) && <span className="absolute bottom-0 right-0 size-3 bg-green-500 rounded-full ring-2 ring-white"></span>}</div>
                              <div className="flex-1 text-left min-w-0"><div className="font-bold text-sm truncate">{friend.fullName}</div><div className="text-xs text-zinc-500">{onlineUsers.includes(friend._id) ? "Online" : "Offline"}</div></div>
                              <div className="btn btn-xs btn-primary shrink-0">Tantang</div>
                          </button>
                      ))}
                  </div>
              </div>
